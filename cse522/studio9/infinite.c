...
int main(int argc, char** argv)
{
	if (argc < 4)
	{
		printf("Usage: %s [core number] [priority] [process count]\n", argv[0]);
		return 1;
	}
	...
	// Fork off the specified number of real-time processes
	int requestedProcessCount = atoi(argv[3]);
	int i;
	for (i = 1; i < requestedProcessCount; ++i)
	{
		if (fork() == 0)
			break;
		else printf("CHILD\n");
